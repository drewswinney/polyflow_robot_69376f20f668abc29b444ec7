  ...
  let
    ...
    workspaceSrcPath =
      ...
      in if builtins.pathExists path then path else
        throw "workspace src not found at ${toString path}";
    ...
    webrtcWorkspace = uv2nix.lib.workspace.loadWorkspace { workspaceRoot = ./workspace/src/webrtc; };

    # Create Python set with webrtc dependencies from uv2nix
    ...
    # Get list of webrtc Python dependencies as packages
    # Extract all dependencies from the Python set (excluding webrtc itself)
    webrtcPythonDeps =
      let
        # Get all packages that were added by the overlay (dependencies from uv.lock)
        allPkgNames = builtins.attrNames webrtcPythonSet;
        # Filter out webrtc itself and check if package exists in base set
        # Use tryEval to avoid errors from removed package aliases
        depNames = builtins.filter (name:
          name != "webrtc" &&
          (builtins.tryEval webrtcPythonSet.${name}).success && (
            !(pyProjectPythonBase ? ${name}) || (
              (builtins.tryEval pyProjectPythonBase.${name}).success &&
              webrtcPythonSet.${name} != pyProjectPythonBase.${name}
            )
          )
        ) allPkgNames;
        deps = builtins.map (dep: webrtcPythonSet.${dep}) depNames;
      in
        builtins.trace "webrtc dependency names: ${lib.concatStringsSep ", " depNames}"
        builtins.trace "webrtc dependency count: ${toString (builtins.length deps)}"
        deps;
...
EOF
    ...
    webrtcSrc = pkgs.lib.cleanSourceWith {
      src = builtins.path { path = ./workspace/src/webrtc; name = "webrtc-src"; };
      filter = path: type:
    ...
    webrtcPkg = pkgs.python3Packages.buildPythonPackage {
      ...
      # Skip runtime dependency check - deps are provided via propagatedBuildInputs
      # Remove the pythonRuntimeDepsCheckHook from nativeCheckInputs
      nativeCheckInputs = [];

      # ROS runtime deps + uv2nix-built Python dependencies
      propagatedBuildInputs = with rosPkgs; [
        rclpy
        launch
        launch-ros
...
EOF
  ...
  in
  {
    ...
    nixosConfigurations.rpi4 = nixpkgs.lib.nixosSystem {
      inherit system;
      specialArgs = {
        inherit webrtcPkg pyEnv robotConsoleStatic robotApiPkg rosWorkspace;
      };
      modules = [
        ({ ... }: {
          nixpkgs.overlays =
